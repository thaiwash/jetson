    ...
    Discourse._registerPluginCode('0.1', function (api) {
      try {
        api.decorateCooked(function ($cooked) {
          var NV_DL_DISCLAIMER_ACCEPTED = "nv_dl_disclaimer_accepted";
          $cooked.on("click", "a.attachment", function (event) {
            if (typeof sessionStorage[NV_DL_DISCLAIMER_ACCEPTED] === 'undefined') {
              var confirmed = confirm("NVIDIA makes no guarantee about the safety of this file. Download attachments at your own risk.");

              if (confirmed) {
                sessionStorage[NV_DL_DISCLAIMER_ACCEPTED] = 1;
              }
